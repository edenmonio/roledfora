(() => {
  ...
  function serviceLabels(item) {
    ...
    const rawServices = [...(Array.isArray(item.servicos) ? item.servicos : []),...(Array.isArray(item.services) ? item.services : []),item.servico,item.service].filter(Boolean);
    return rawServices.map(raw => {
      const found = list.find(([id,label]) => norm(raw) === norm(id) || norm(raw) === norm(label));
      return lower(found?.[1] || raw);
    });
  }
  ...
  function card(item) {
    ...
    const handle = lower(item.instagramHandle || '');
    const contactLabel = handle || 'ver perfil / contato';

    return `<article class="card professional-card">
      <span class="card-kind">${esc(areaLabel(item))}</span>
      <h3>${esc(lower(item.nome))}</h3>
      <div class="card-meta">
        ${physicalLocation ? `<p>📍 ${esc(physicalLocation)}</p>` : ''}
        ${item.online ? '<p>💻 atendimento online</p>' : ''}
        ${services.length ? `<p>🛠️ ${esc(services.join(' · '))}</p>` : ''}
      ...
      ${link ? `<div class="actions"><a href="${esc(link)}" target="_blank" rel="noopener">${esc(contactLabel)}</a></div>` : ''}
    </article>`;
  }
  ...
  document.addEventListener('change',event => {
    ...
    if (!select) return;
    if (select.id === 'prof-tipo') fillServices(select.value);
    render();
  });
